...
[[ -n "${RUN_DIR}" && -n "${BASE_RUN_DIR}" && -n "${EVALUATION_ROOT}" ]] || {
  echo "run/base/evaluation directories required" >&2
  exit 2
}
...
[[ -d "${EVALUATION_DATA_ROOT}" ]] || { echo "missing evaluation dataset root: ${EVALUATION_DATA_ROOT}" >&2; exit 1; }
[[ -z "$(cd -- "${REPO_ROOT}" && git status --porcelain --untracked-files=all)" ]] || {
  echo "production worktree must be clean before Qwen" >&2; exit 1;
}

mapfile -t PREFLIGHT < <("${PYTHON_BIN}" - "${VALIDATION}" "${DECISIONS}" "${TRACES}" "${KEYFRAMES}" "${SOURCE_BUNDLE}" "${BASE_PROVENANCE}" "${EVALUATION_DATA_ROOT}" <<'PY'
import hashlib,json,pathlib,re,sys
validation_path,decisions,traces,keyframes,source_bundle,base_path,evaluation_data_root=map(pathlib.Path,sys.argv[1:])
...
for row in members:
    ...
    expected=(evaluation_data_root/path.name).resolve(strict=True)
    if path != expected: raise SystemExit(f"source member is not bound to evaluation dataset root: {path}")
...
"${PYTHON_BIN}" - "${RUNTIME_PROVENANCE}" "${BASE_PROVENANCE}" "${BASE_PROVENANCE_SHA}" "${VALIDATION_SHA}" "${WFS_SOURCE_SHA}" "${LMMS_SOURCE_SHA}" "${QWEN_REQUESTED}" "${QWEN_SNAPSHOT}" "${QWEN_SIGNATURE}" "${PACKAGE_SIGNATURE}" "$(cd -- "${REPO_ROOT}" && git rev-parse HEAD)" "${REPO_ROOT}" "${EVALUATION_ROOT}" "${EVALUATION_DATA_ROOT}" <<'PY'
import json,os,pathlib,sys
(output,inherited,inherited_sha,validation_sha,wfs_sha,lmms_sha,requested,snapshot,checkpoint_signature,packages,head,production_root,evaluation_root,evaluation_data_root)=sys.argv[1:]
payload={"schema_version":1,"inherited_base_provenance":str(pathlib.Path(inherited).resolve()),"inherited_base_provenance_sha256":inherited_sha,"validation_sha256":validation_sha,"wfs_git_head":head,"wfs_code_sha256":wfs_sha,"lmms_source_sha256":lmms_sha,"production_repo_root":production_root,"evaluation_root":evaluation_root,"evaluation_data_root":evaluation_data_root,"checkpoint_requested":requested,"checkpoint_snapshot_path":snapshot,"checkpoint_snapshot_revision":pathlib.Path(snapshot).name,"checkpoint_signature":checkpoint_signature,"runtime":json.loads(packages),"grid":{"methods":["canonical_uniform","phasefuse_rc12"],"origins":[0,1,2,3,4],"independent_cells":10,"uniform_inference_reused":False},"generation":{"max_num_frames":16,"max_pixels":200704,"attention":"sdpa","batch_size":1}}
...
bash "${SCRIPT_DIR}/run_mllm_grid.sh" \
  ...
  --repo-root "${EVALUATION_ROOT}" --predictions-output "${RUN_DIR}/primary_predictions.raw.jsonl" \
