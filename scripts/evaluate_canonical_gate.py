#!/usr/bin/env python3
"""Evaluate the frozen canonical-residual downstream safety gate."""
...
def evaluate_gate(summary: dict, treatment_method: str) -> dict:
    ...
    if (
        ...
        or stability.get("treatment_method") != treatment_method
    ):
        raise ValueError("downstream arm direction is not treatment minus canonical")
    ...
    return {
        ...
        "effect_definition": f"{treatment_method} - canonical_uniform",
...
def main() -> None:
    ...
    if result["status"] != "pass":
        raise SystemExit("frozen canonical-residual downstream safety gate failed")
